...
| Audit Web entry | `apps/audit-web/src/App.tsx` | implemented Chinese workbench | `思而听二进制漏洞审计平台`, Chinese analysis status summary, Chinese branch command, Chinese run cancel command, Chinese approval commands |
| Audit Web data owner | `apps/audit-web/src/lib/workbenchData.ts` | implemented Chinese mock workbench | `createMockWorkbench`, `branchFromCheckpoint`, `approveInterrupt`, `rejectInterrupt`, `cancelRun`, Chinese mock finding and next action copy |
