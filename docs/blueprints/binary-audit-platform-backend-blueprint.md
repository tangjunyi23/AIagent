...
基于 LangGraph 二次开发一个面向二进制审计的多 agent 后端。后端必须让 AI 具备真实执行、验证和审计能力，而不是只输出自然语言推测。平台应平均支持 ELF、PE、Mach-O、APK、固件、压缩包和未知二进制等主流输入格式，覆盖 CTF 解题、安全风险分析、恶意软件分析、固件逆向、代码审计、移动安全等多类场景。固件模拟只是高风险动态能力之一，不应成为默认中心路径；EMBA/QEMU 与 Frida、沙箱运行、CTF 本地验证、恶意软件离线行为分析等能力同级，都必须通过 policy/interrupt 控制。

平台通过 LangGraph 管理长运行状态、多 agent 协作、checkpoint、interrupt、失败恢复和工具编排，通过 MCP/工具服务接入 Ghidra、IDA/idat、objdump、jadx、binwalk、Joern、EMBA、QEMU、Frida、YARA、capa、pwntools 等专业工具链。所有关键成果必须结构化为 artifact、finding、report、approval、event 或 result 记录，包括 Flag 提取结果、教学 PoC、IoC 威胁情报报告、YARA 规则、漏洞发现、SBOM、反编译结果和审计报告。
...
Frontend
  ...
  -> Audit Platform API
       -> LangGraph Agent Server
            -> Supervisor Graph
                 -> Triage Agent
                 -> Format Router Agents
                 -> Static Reverse Agents
                 -> CTF Agent Team
                 -> Malware / Threat Agent Team
                 -> Mobile Agent Team
                 -> Firmware Agent Team
                 -> Code Audit Agent Team
                 -> Dynamic / Sandbox Agents
                 -> Exploit / Verification Agents
                 -> Report Agent
       -> Tool Orchestrator
            -> MCP Tool Servers
            -> Container Sandbox Workers
            -> VM/QEMU/EMBA/Frida/YARA Workers
       -> Storage
...
- 新增 `apps/audit-workers`：工具执行 worker、沙箱执行、Frida/YARA/EMBA/QEMU 等长任务和格式工具 adapter。
...
- **Subgraph**：格式和场景解耦建模。格式子图负责 ELF、PE、Mach-O、APK、固件等解析；场景子图负责 CTF、安全风险、恶意软件、固件逆向、代码审计、移动安全等任务目标。
...
- **AuditLog**：所有敏感操作审计。
- **StructuredResult**：按场景沉淀的结构化成果，例如 Flag 提取、教学 PoC、IoC 报告、YARA 规则、SBOM、漏洞验证结论和教学解释。
...
`StructuredResult` 应至少包含：

- `kind`：`flag-result`、`teaching-poc`、`ioc-report`、`yara-rule`、`vulnerability-finding`、`sbom`、`audit-summary` 等。
- `status`：`draft`、`needs-review`、`verified`、`rejected`。
- `artifact_ids`：支撑该成果的 artifact 列表。
- `finding_ids`：关联漏洞或风险发现。
- `confidence`：0 到 1 的置信度。
- `safety_notes`：是否需要审批、是否包含敏感内容、是否可导出。
...
START
  ...
  -> triage_router
  -> format_router
      -> elf_pe_macho_subgraph
      -> apk_subgraph
      -> firmware_subgraph
      -> archive_or_unknown_subgraph
  ...
  -> scenario_router
      ...
      -> risk_subgraph
      -> code_audit_subgraph
      -> mobile_security_subgraph
      -> firmware_reverse_subgraph
  -> verification_router
  -> structured_result_builder
  -> report_builder
  -> END
```
...
- **Triage Agent**：文件类型识别、架构识别、加壳/压缩/加密判断、格式与场景路由、工具链推荐。
- **Reverse Agent**：面向 ELF、PE、Mach-O 和 native `.so` 调用 Ghidra/idat/objdump/dumpbin/otool，抽取函数、字符串、CFG、导入导出、伪代码。
- **CTF Agent**：面向 pwn/reverse 任务生成 solver、教学 PoC、运行日志和 Flag 提取结果；远程验证必须确认授权。
- **Malware / Threat Agent**：静态 IoC、反沙箱、C2、行为簇、YARA 建议、IoC 威胁情报报告，默认禁止公网外连。
- **Firmware Agent**：固件解密/解包、rootfs 建模、服务发现、EMBA/Joern/QEMU 编排；模拟为可选高风险路径，不作为默认必经步骤。
- **Mobile Agent**：jadx/apktool 输出分析、Manifest 风险、导出组件、敏感 API、动态 hook 建议。
- **Code Audit Agent**：面向 C/C++、脚本、JNI、CGI、Lua/PHP/Shell 和项目源码做 source/sink、依赖与配置风险分析。
- **Dynamic Agent**：沙箱执行、抓包、HTTP 流量、服务探测、行为观测和受控验证。
- **Exploit/Verifier Agent**：对候选漏洞做最小化验证，必须遵循授权策略和审批；教学 PoC 默认只面向本地样本或 CTF 授权环境。
- **Result Builder Agent**：把 Flag、教学 PoC、IoC、YARA、SBOM、漏洞验证、审计摘要写入 `StructuredResult` 和 artifact。
...
| `otool`, `jtool2`, `codesign`, `lipo` | Mach-O | load commands、签名、架构切片、符号 |
...
| YARA/capa/Sigma helper | 恶意软件/威胁情报 | YARA 规则、能力标签、检测建议 |
| pwntools/checksec/ROPgadget | CTF | 利用条件、ROP、教学 PoC、Flag 证据 |
...
格式路由和场景路由必须解耦，避免把固件流程默认套到所有样本上：

```text
sample_intake
  -> format_router: ELF | PE | Mach-O | APK | Firmware | Archive | Unknown
  -> scenario_router: CTF | risk | malware | firmware-reverse | code-audit | mobile-security
  -> result_targets: finding | flag-result | teaching-poc | ioc-report | yara-rule | sbom | report
```

默认策略：

- 每个样本先完成轻量 triage，再按格式选择最小必要工具链。
- 任务目标决定是否进入 CTF solver、恶意软件 IoC、移动安全、代码审计或固件逆向子图。
- 固件模拟、恶意样本动态执行、CTF 远程验证、漏洞利用验证和外连网络都属于可选高风险动作，必须经 policy/interrupt。
- 所有场景都必须能产生结构化成果；自然语言报告只是最终呈现层。
...
固件流程是多格式平台的一条可选格式路径，不应高于 ELF、PE、Mach-O 或 APK。默认先产出解包、rootfs、组件清单和静态风险；EMBA/QEMU 模拟只在任务目标需要且用户审批后进入。
...
- 输出 `threat.ioc_report`、`threat.yara_rule`、`threat.behavior_chain` 和防御建议 artifact。
...
- Exploit 结果必须保存教学 PoC、运行日志和 flag 证据。
- 输出 `ctf.flag_result` 和 `exploit.teaching_poc`，并记录是否经过本地/远程授权验证。

### 8.6 代码审计

- 支持 C/C++、JNI、CGI、Lua、PHP、Shell、Java/Kotlin 反编译源码和配置文件审计。
- Joern/semgrep-like normalizer 产出 source/sink、危险 API、配置风险、凭据泄露和依赖风险。
- 输出 `code.audit_summary`、`vuln.finding_evidence` 和修复建议 artifact。

### 8.7 结构化成果类型

| 成果 | Artifact Type | 典型场景 | 安全边界 |
| --- | --- | --- | --- |
| Flag 提取结果 | `ctf.flag_result` | CTF pwn/reverse | 仅限授权目标；远程验证需记录目标和审批 |
| 教学 PoC | `exploit.teaching_poc` | CTF、漏洞教学、企业复现 | 默认最小化、本地化、可解释；禁止自动攻击未授权目标 |
| IoC 威胁情报报告 | `threat.ioc_report` | 恶意软件、威胁分析 | 默认脱敏，外连与动态运行需要审批 |
| YARA 规则 | `threat.yara_rule` | 恶意软件、样本聚类、防御检测 | 规则需附样本证据和误报说明 |
| 代码审计摘要 | `code.audit_summary` | 代码审计、固件 Web/CGI、移动 native | 关联 source/sink 和修复建议 |
| 漏洞证据 | `vuln.finding_evidence` | 全部场景 | 高危 finding 至少关联一个证据 artifact |
...
- `tool.joern.taint`
- `tool.yara.generate_rule`
- `tool.ctf.solve_local`
- `tool.threat.extract_ioc`
- `tool.emba.run`
...
- Kubernetes。
- API server 水平扩展。
- LangGraph queue workers 按资源特征拆池：general、binary-reverse、mobile、malware、firmware、dynamic、report。
- Ghidra/IDA/Frida/YARA/EMBA/QEMU worker 使用 node selector 和资源配额；固件模拟 worker 不应占用默认队列。
...
- Ghidra/Joern/Frida/EMBA/QEMU CPU/内存密集任务：较低并发，并通过专用 worker 池调度。
...
- CTF toy pwn/reverse。
- 恶意软件离线 IoC/YARA 测试样本。
- 代码审计 source/sink toy project。

### 13.3 E2E 验收

- 上传样本后 30 秒内完成 triage。
- 工具失败不导致整体分析卡死，能降级并产出可解释失败原因。
- ELF/PE/Mach-O/APK/固件任一格式都能产出基础 triage artifact。
- 固件标准流程能产出 rootfs tree、服务列表、至少一个静态 finding，但不要求默认进入模拟。
- CTF 流程能产出 `ctf.flag_result` 或明确说明未解出的结构化原因。
- 恶意软件流程能产出 `threat.ioc_report` 和候选 `threat.yara_rule`。
- 代码审计流程能产出 source/sink 证据或结构化无发现说明。
...
- 实现 binutils、Mach-O inspect、jadx、binwalk/unblob、YARA/capa、Ghidra headless adapter。
- 实现 tool execution DB record 和 artifact normalizer。
- 为 Joern、Frida、YARA、EMBA、QEMU 和 CTF 本地验证预留接口。
...
- ELF/PE/Mach-O/APK/固件五类样本入口，权重均衡，均支持基础 triage 和 artifact 输出。
- 工具接入：binutils、Mach-O inspect、jadx、binwalk/unblob、YARA/capa、Ghidra headless。
- LangGraph 多 agent：supervisor、triage、reverse、ctf-lite、malware-lite、firmware-lite、mobile-lite、code-audit-lite、report、watchdog。
- interrupt：动态执行、远程验证、恶意样本运行、固件模拟、敏感 artifact 导出审批。
- 固件：MVP 只做解包、rootfs tree、基础静态风险；EMBA 在 MVP 中实现异步任务接口、权限审批和 artifact 导入，深度模拟放入后续里程碑。
- 结构化成果：至少保留 `ctf.flag_result`、`exploit.teaching_poc`、`threat.ioc_report`、`threat.yara_rule` 的 schema 和 mock artifact 能力。
- 报告：Markdown/HTML 导出。
